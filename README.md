<p align="center">
  <img src="assets/umbra-header-v5.jpg" alt="Umbra" width="850">
</p>
...
umbra/
  src/
    ...
    state.rs                Chain state (bonds, slashing, sled-backed nullifiers, contract registry, contract state), Ledger, parallel verification
    ...
    crypto/
      ...
      stark/
        ...
        types.rs            BalanceStarkProof, SpendStarkProof, ExecutionStarkProof, witness types
        ...
        execution_air.rs    AIR for contract execution proofs (VM trace constraints)
        execution_prover.rs Prover for contract execution STARK proofs
        verify.rs           STARK verification wrappers
    transaction/
      mod.rs                Transaction types (Transfer, ContractDeploy, ContractCall), validation
      builder.rs            TransactionBuilder API for constructing transactions
    vm/
      mod.rs                Contract VM module root, re-exports
      instruction.rs        Opcode enum (Add, Sub, Mul, Div, Const, Eq, CJump, Jump, Load, Store, Hash, etc.)
      executor.rs           Register-based VM over Goldilocks field, trace recording for STARK proving
      memory.rs             Fixed-size memory model with bounds checking
      contract.rs           ContractCode, ContractId, state commitment hashing
    ...
    node/
      ...
      storage.rs            Persistent storage trait + sled backend (vertices, txs, validators, contracts, contract state)
    network/
...
**~41,000 lines of Rust** across 51 source files with **1252 tests**.
...
STARK-provable smart contract execution via a register-based virtual machine:

- **Contract VM** — 16 general-purpose registers over the Goldilocks field (p = 2^64 - 2^32 + 1), bounded execution (64K steps, 16K memory), trace recording for STARK proving
- **Opcodes** — arithmetic (Add, Sub, Mul, Div), constants, equality, control flow (Jump, CJump), memory (Load, Store), cryptographic (Hash via Rescue Prime), I/O (LoadInput, EmitOutput, EmitNullifier), termination (Halt, Fail)
- **Execution AIR** — Winterfell AIR constraints verify correct VM execution; proofs are generated and verified using the same STARK infrastructure as balance/spend proofs
- **Contract deployment** — `ContractDeploy` transactions register validated bytecode on-chain; contracts are identified by `hash_domain("umbra.contract_id", bytecode)`
- **Contract calls** — `ContractCall` transactions include a STARK execution proof; the chain verifies the proof and cross-checks contract_id, function_hash, output commitments, and emitted nullifiers
- **Persistent state** — per-contract state hash (`[u8; 32]`) stored on-chain; on each call, the chain verifies the proof's `initial_state_hash` matches the stored hash, then updates to `final_state_hash`. Actual memory contents remain off-chain (the STARK proof cryptographically binds them to the hashes)
...
- **10 named trees**: `vertices`, `transactions`, `nullifiers`, `chain_meta`, `commitment_levels`, `validators`, `finalized_index`, `coinbase_outputs`, `contracts`, `contract_states`
...
- `ChainStateMeta` captures full chain state snapshots (epoch, roots, counts, finalized count, total minted, contract count) for persistence
...
```bash
cargo test                       # Full suite (~1252 tests)
...
- **Rescue Prime + BLAKE3-512** — commitments, hybrid blake3_512_binding generation and verification, field element conversions, state_digest_to_hash extraction, hash_to_felts large value reduction, felts_to_hash field-native preservation, exp7 zero and one edge cases, domain separation cross-function uniqueness (all 4 hash functions produce different outputs for same inputs), merge non-commutativity, commitment nonzero output from zero inputs, init state domain tag correctness, init state input placement verification
- **Nullifiers** — determinism, double-spend detection, to_felts roundtrip, as_bytes, NullifierSet len/is_empty/contains/remove/iter
- **Merkle tree** — construction, path verification, depth-20 canonical padding, restore from stored level data, last-appended path coverage, truncate_to_zero, truncate_partial (root matches fresh build), truncate_noop_when_larger, truncate_then_reappend, build_merkle_tree_empty, level_len, path_out_of_bounds, compute_merkle_root_checked correct/wrong depth, tampered sibling produces different root, restore internal consistency repair
...
- **STARK AIR and prover coverage** — balance AIR (trace length for 0/1/max IO, power-of-two, width=86, periodic columns count, assertion count consistency, range bit field overflow prevention), spend AIR (trace length power-of-two with padding, width=30, depth=20, total blocks, periodic columns count and lengths, assertion count consistency, proof_link block index), Goldilocks prime rejection (balance/spend from_bytes reject values >= p, accept just below p, public inputs element ordering), balance prover (link nonce count mismatch error, correct trace dimensions with COMMITMENT_DOMAIN at block 0, net balance accumulation column), spend prover (wrong Merkle depth rejection, correct depth acceptance, wrong nullifier/proof_link rejection, domain tags at correct positions, commitment register constancy)
...
- **Contract RPC** — GET /contract/:id (found, not found, invalid hex), GET /contract/:id/exists
...
- **Minimum validator count** — deregistration is rejected when active validators would drop to `MIN_VALIDATORS` (4), preserving BFT quorum guarantees
- **Epoch validation on vertices** — DAG insertion verifies `epoch == round / EPOCH_LENGTH`, preventing epoch-spoofing attacks that could bypass committee checks
- **Consensus message sanitization** — `sanitize()` truncates `BftCertificate.signatures`, `FinalizedVerticesResponse.vertices`, and `EpochStateResponse.committee` to protocol limits, preventing oversized consensus payloads from consuming unbounded memory
- **Next-round vote signature verification** — votes buffered for future rounds are signature-verified before storage, preventing memory exhaustion from forged votes
- **State root verification on restore** — `restore_from_storage()` recomputes the state root after reconstruction and rejects mismatches, detecting storage corruption or tampering
- **Cross-epoch equivocation detection** — BFT state preserves historical committee lists (capped at `MAX_COMMITTEE_HISTORY`), allowing equivocation evidence to be verified even after epoch transitions
- **Sync-state guards on consensus participation** — voting and vertex proposal are suppressed while the node is syncing, preventing partially-synced nodes from casting invalid votes
- **View-change round advancement** — when staleness is detected (no finalization within timeout), the BFT round is advanced to enable view changes and prevent permanent liveness stalls
- **VRF domain separation** — `compute_id` includes a presence flag byte (0/1) before optional VRF data, preventing collisions between vertices with and without VRF proofs
- **Idempotent slashing** — `slash_validator()` short-circuits if the validator is already slashed, preventing double-penalization from replayed equivocation evidence
- **Atomic slashing before flush** — equivocation processing runs before the storage batch is flushed, ensuring slashing state is persisted atomically with vertex finalization
...
- **Stealth intermediate zeroization** — `derive_one_time_key` and `derive_spend_auth` explicitly zeroize their 36-byte intermediate buffers after use, preventing secret material from lingering in memory
- **Plaintext zeroization** — all four encrypt/decrypt methods in `encryption.rs` zeroize the padded plaintext buffer after AEAD operations, preventing cleartext from lingering in memory after encryption or decryption
...
- **Contract bytecode validation** — `ContractCode::new()` validates that bytecode is non-empty, does not exceed `MAX_CONTRACT_SIZE` (64 KiB), and ends with a `Halt` instruction, preventing deployment of malformed or non-terminating contracts
- **Contract execution bounds** — the VM enforces `MAX_STEPS` (65,536) and `MAX_MEMORY` (16,384 field elements), preventing unbounded computation and memory exhaustion from malicious contracts
- **Contract state hash verification** — `apply_transaction_unchecked()` verifies the execution proof's `initial_state_hash` against the stored on-chain state hash using `constant_time_eq`, rejecting calls with stale or fabricated initial state; the check is performed sequentially in Pass 2 to support correct chaining of multiple calls to the same contract within a single block
- **Contract state persistence** — per-contract state hashes are stored in a dedicated sled tree and included in `state_root()`, ensuring state consistency across restarts and enabling detection of storage corruption via state root divergence
- **Execution proof binding** — contract call transactions verify the execution proof's `contract_id` and `function_hash` match the transaction fields during `validate_structure()`, preventing proof transplant attacks across contracts or functions
- **Contract transaction size limits** — contract transactions are capped at `MAX_CONTRACT_TX_SIZE` (256 KiB) in the mempool, preventing memory exhaustion from large execution proofs
- **Contract registry limits** — the chain enforces `MAX_CONTRACTS` (100,000), preventing unbounded state growth from contract deployment spam
