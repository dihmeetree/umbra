...
describe('Token Contract', () => {
  ...
  it('should mint and track balances', () => {
    ...
    const tokenNames = new Map([
      [tokenColorType.toString(), 'MyToken']
    ]);
  ...
  it('should handle transfers', () => {
    ...
    simulator
      .as({ secretKey: adminWallet.secretKey })
      .executeImpureCircuit('transfer', encodeCoinInfo(coin!), userWallet.coinPublicKey, 500n);
...
describe('Multi-Contract Interaction', () => {
  ...
  beforeEach(() => {
    ...
    tokenSimulator = ContractSimulator.deploy(
      new TokenContract.Contract({}),
      {
        initialPrivateState: { secretKey: adminWallet.secretKey },
        nonce: generateNonce(),
    ...
    // Deploy DEX contract
    dexSimulator = ContractSimulator.deploy(
      new DexContract.Contract({}),
      {
        initialPrivateState: { secretKey: adminWallet.secretKey },
        nonce: generateNonce(),
        coinPublicKey: adminWallet.coinPublicKey,
...
// Compare
const changes = balanceTracker.getBalanceChanges('after',
...
const privateState = WalletManager.createPrivateState<MyContractPrivateState>(wallet);
