...
export class LedgerInspector<TPrivateState> {
  ...
  private createLedgerAccessor(): any {
    ...
    try {
      ...
      logger.warn(
        'Could not create ledger accessor from contract, using raw ledger'
      )
    }
  ...
  getMerkleTreeInfo(treeName: string): MerkleTreeInfo | null {
    try {
      ...
      logger.error(
        `Error inspecting Merkle tree '${treeName}':`,
  ...
  hasCommitmentInHistory(treeName: string, commitment: Uint8Array): boolean {
    try {
      ...
      logger.debug(
        `Error checking commitment in ${treeName}:`,
        (error as Error).message
      )
      return false
    }
  ...
  isInSet(setName: string, value: Uint8Array): boolean {
    try {
      ...
      logger.debug(
        `Error checking set membership in ${setName}:`,
        (error as Error).message
      )
      return false
    }
  ...
  getGlobalState(fieldName: string): any {
    try {
      ...
      logger.error(
        `Error reading global state '${fieldName}':`,
        (error as Error).message
      )
      return undefined
    }
    ...
    if (trees.length > 0) {
      ...
      for (const treeName of trees) {
        ...
        if (info) {
          ...
          console.log(
            `      Root: ${info.root.field.toString().slice(0, 16)}...`
          )
