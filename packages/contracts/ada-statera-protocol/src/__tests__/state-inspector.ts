...
import type { StateraPrivateState, AdminMetadata, StakeMetadata } from '../index.js';
...
export class StateInspector {
   ...
   */
  private diffStates(before: StateraPrivateState, after: StateraPrivateState): StateDiff[] {
    ...
    if (before.stake_metadata.effectiveBalance !== after.stake_metadata.effectiveBalance) {
      diffs.push({
    ...
    if (before.stake_metadata.stakeReward !== after.stake_metadata.stakeReward) {
      diffs.push({
    ...
    if (before.admin_metadata.protocolFeePool !== after.admin_metadata.protocolFeePool) {
      diffs.push({
    ...
    return JSON.stringify(
      {
        ...
        stakeMetadata: {
          ...
          entryADASUSDIndex: state.stake_metadata.entry_ADA_SUSD_index.toString(),
 ...
 */
export function inspectState(simulator: ContractSimulator<StateraPrivateState>): StateInspector {
 ...
 */
export function logState(simulator: ContractSimulator<StateraPrivateState>, label?: string): void {
