...
describe('Ada Statera Protocol - Full Integration Tests', () => {
  ...
  describe('Stability Pool Operations', () => {
    ...
    it('should check and withdraw stake rewards', () => {
      ...
      new AdminBuilder(simulator, adminWallet).resetConfig(70n, 68n, 120n)
      ...
      const stakerMetadata = stateManager.getState(
        staker,
        'after-stake'
      ).stake_metadata
      ...
      if (updatedMetadata.stakeReward > 0n) {
        const withdrawAmount =
          updatedMetadata.stakeReward > 100n
            ? 100n
            : updatedMetadata.stakeReward
  ...
  describe('Liquidation Flow', () => {
    it('should liquidate under-collateralized position', () => {
      ...
      simulator
        .as(
          createPrivateStateForWallet(targetUser, simulator),
          targetUser.coinPublicKey
        )
      ...
      simulator
        .as(
          getPrivateStateAfterDeposit(simulator, targetUser, depositAmount),
          targetUser.coinPublicKey
        )
  ...
  describe('Collateral Withdrawal', () => {
    it('should allow withdrawing excess collateral', () => {
      const { simulator, adminWallet, userWallets, collateralTokenType } =
        fixture
      const user = userWallets[0]
      const oraclePk = createMockOraclePk()
  ...
  describe('Protocol Pause Functionality', () => {
    it('should prevent operations when paused', () => {
      ...
      // Pause protocol
      asAdmin(simulator, adminWallet).executeImpureCircuit('togglePause')
  ...
  describe('Multi-User Scenarios', () => {
    it('should handle multiple users with different positions', () => {
      ...
      const user1Compliance = createMockComplianceToken(
        user1.coinPublicKey,
        oraclePk
      )
      ...
      expect(() => {
        simulator
          .as(
            createPrivateStateForWallet(user1, simulator),
            user1.coinPublicKey
          )
          .executeImpureCircuit(
            'depositToCollateralPool',
      ...
      const user2Compliance = createMockComplianceToken(
        user2.coinPublicKey,
        oraclePk
      )
      ...
      expect(() => {
        simulator
          .as(
            createPrivateStateForWallet(user2, simulator),
            user2.coinPublicKey
          )
          .executeImpureCircuit(
            'depositToCollateralPool',
      ...
      expect(() => {
        simulator
          .as(
            createPrivateStateForWallet(user3, simulator),
            user3.coinPublicKey
          )
  ...
  describe('Protocol Fee Accumulation', () => {
    it('should accumulate fees from borrowing and redemption', () => {
      ...
      const userCompliance = createMockComplianceToken(
        user.coinPublicKey,
        oraclePk
      )
      ...
      simulator
        .as(
          getPrivateStateAfterDeposit(simulator, user, 5000n),
          user.coinPublicKey
        )
