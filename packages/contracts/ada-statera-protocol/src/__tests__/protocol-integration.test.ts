...
describe('Ada Statera Protocol - Full Integration Tests', () => {
  ...
  describe('Complete Deposit → Mint → Repay Flow', () => {
    it.skip('should allow user to deposit collateral, mint sUSD, and repay debt', () => {
      ...
      try {
        ...
        console.log('✅ Deposit circuit executed successfully');
      } catch (e) {
        console.log('❌ Deposit circuit FAILED:', (e as Error).message);
        throw e;
      }

      // Debug: Check tree root after deposit
      console.log('\n=== After Deposit (before mint) ===');
      console.log('About to call mint_sUSD...');
      console.log('====================================\n');
  ...
  describe('Stability Pool Operations', () => {
    it('should allow staking to stability pool', () => {
      ...
      expect(() => {
        simulator
          .as(createPrivateStateForWallet(staker), staker.coinPublicKey)
          .executeImpureCircuit(
    ...
    it.skip('should check and withdraw stake rewards', () => {
      const { simulator, userWallets, sSUSDTokenType } = fixture;
      const staker = userWallets[0];
      ...
      expect(() => {
        simulator
          .as(getPrivateStateAfterStake(simulator, staker, stakeAmount), staker.coinPublicKey)
    ...
    it.skip('should allow withdrawing stake', () => {
      const { simulator, userWallets, sSUSDTokenType } = fixture;
      const staker = userWallets[0];
  ...
  describe('Liquidation Flow', () => {
    it.skip('should liquidate under-collateralized position', () => {
      const { simulator, adminWallet, userWallets, collateralTokenType, sSUSDTokenType } = fixture;
      ...
      simulator
        .as(createPrivateStateForWallet(targetUser), targetUser.coinPublicKey)
        .executeImpureCircuit(
          'depositToCollateralPool',
          mockCoin,
          depositAmount,
      ...
      simulator
        .as(getPrivateStateAfterDeposit(simulator, targetUser, depositAmount, collateralTokenType), targetUser.coinPublicKey)
        .executeImpureCircuit('mint_sUSD', mintAmount);
  ...
  describe('Redemption Flow', () => {
    it.skip('should allow sUSD redemption for collateral', () => {
  ...
  describe('Collateral Withdrawal', () => {
    it.skip('should allow withdrawing excess collateral', () => {
  ...
  describe('Multi-User Scenarios', () => {
    it('should handle multiple users with different positions', () => {
      ...
      expect(() => {
        simulator
          .as(createPrivateStateForWallet(user1), user1.coinPublicKey)
          .executeImpureCircuit(
            'depositToCollateralPool',
            user1Coin,
            user1Deposit,
      ...
      expect(() => {
        simulator
          .as(createPrivateStateForWallet(user2), user2.coinPublicKey)
          .executeImpureCircuit(
            'depositToCollateralPool',
            user2Coin,
            user2Deposit,
      ...
      expect(() => {
        simulator
          .as(createPrivateStateForWallet(user3), user3.coinPublicKey)
  ...
  describe('Protocol Fee Accumulation', () => {
    it.skip('should accumulate fees from borrowing and redemption', () => {
      const { simulator, adminWallet, userWallets, collateralTokenType, sSUSDTokenType } = fixture;
      const user = userWallets[0];
      const oraclePk = createMockOraclePk();
      ...
      simulator
        .as(getPrivateStateAfterDeposit(simulator, user, 5000n, collateralTokenType), user.coinPublicKey)
